...
exports.isAccounting = () => {
  return (req, res, next) => {
    const decodedToken =
      this.verify(utils.jwtFrom(req)) && this.decode(utils.jwtFrom(req));
    if (
      decodedToken &&
      decodedToken.data &&
      decodedToken.data.role === exports.roles.accounting
    ) {
      next();
    } else {
...
exports.isDeluxe = (req) => {
  ...
  return (
    decodedToken &&
    decodedToken.data &&
    decodedToken.data.role === exports.roles.deluxe &&
    decodedToken.data.deluxeToken &&
    decodedToken.data.deluxeToken === this.deluxeToken(decodedToken.data.email)
  );
};
...
exports.isCustomer = (req) => {
  ...
  return (
    decodedToken &&
    decodedToken.data &&
    decodedToken.data.role === exports.roles.customer
  );
};
...
exports.appendUserId = () => {
  return (req, res, next) => {
    try {
      req.body.UserId =
        this.authenticatedUsers.tokenMap[utils.jwtFrom(req)].data.id;
      next();
    } catch (error) {
